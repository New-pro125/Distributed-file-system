...
func (m *MasterTracker) checkAndReplicate() {
	...
	for fileName, records := range m.table {
		...
		if src == nil {
			log.Printf("[MasterTracker] No alive source for file %s, skipping", fileName)
			continue
		}

		needed := m.DesiredReplicas - aliveCount
		for i := 0; i < needed; i++ {
			dst := m.selectMachineToCopyTo(fileName, records)
			if dst == nil {
...
func (m *MasterTracker) selectMachineToCopyTo(fileName string, records []FileRecord) *NodeInfo {
	...
	for _, node := range m.nodes {
		if node.IsAlive && !holders[node.NodeID] {
			return node
		}
	}
	return nil
